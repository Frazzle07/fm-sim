...
**Loose Ball**
A ball with no holder that is either rolling (still decelerating) or at rest, collectable by whichever player reaches it first. When pass flight completes (`t >= 1`), the ball does not stop dead at the **Pass Target** — it inherits a velocity derived from the pass vector and continues rolling, decelerating each tick via a friction constant (`LOOSE_BALL_FRICTION = 0.12`) until it reaches near-zero speed or is collected. Initial velocity = `(passVector / durationMs) * LOOSE_BALL_VELOCITY_FRACTION`, so longer/faster passes arrive with more pace and short passes trickle in — the `passSpeedFactor` randomness already baked into `durationMs` means heavy touches naturally arrive slower. The ball rolls in the same direction as the pass (no angular deviation — the **Pass Target** already carries noise from `LEAD_BEARING_NOISE`). Possession transfers on proximity each tick (same `LOOSE_BALL_COLLECT_RADIUS` check), so the ball is almost always collected mid-roll; coming to a full stop is only a safety fallback. Both teams actively pursue a loose ball (see **LooseBallAction**). This makes **Contested Arrival** literal: the receiver must actively control a moving ball rather than waiting for it to stop at their feet.
...
**ReceiveAction**
The movement behaviour for the player a pass is currently inbound to (`ctx.ballReceiverId`). Highest-priority movement action while the ball is in flight, replacing the simulator's former practice of pinning the receiver in place. While the ball is in flight the receiver runs to the **Pass Target** (the landing point), claiming the space. Once flight ends and the ball is loose, the receiver switches to chasing the ball's live position each tick — naturally intercepting the rolling ball mid-path rather than waiting at a static point. There is no separate "come short" prediction logic; the receiver simply follows the ball as it moves toward them.
